...
public final class InferenceCore {
    ...
    // CPU forward pass removed to avoid confusion - OLMoE only uses GPU path via forwardTornadoVMOlmoe
    ...
    public static FloatArray forwardTornadoVMOlmoe(Model model, OlmoeState state, int token, int position, TornadoVMMasterPlan tornadoVMPlan) {
        ...
        // Execute TornadoVM preprocessing (input setup)
    ...
    /**
     * Apply Rotary Position Embedding (RoPE) to query and key vectors using NEOX-style pairing.
     * This is specifically for OLMoE models which use LLAMA_ROPE_TYPE_NEOX.
     */
    private static void applyRoPEOLMoE(float[] query, float[] key, int position, int headDim, int numHeads,
                                float[] freq_cis_real, float[] freq_cis_imag) {
        // Apply RoPE to each head
        for (int head = 0; head < numHeads; head++) {
            int headOffset = head * headDim;

            // CRITICAL FIX: Use NEOX-style RoPE for OLMoE (pairs are (x_i, x_{i+d/2}) not (x_i, x_{i+1}))
            // This matches llama.cpp's LLAMA_ROPE_TYPE_NEOX implementation
            for (int i = 0; i < headDim / 2; i++) {
                int freqIndex = position * (headDim / 2) + i;
                if (freqIndex < freq_cis_real.length) {
                    ...
                    if (position <= 1 && head == 0 && i <= 4) {
                        System.err.printf("[ROPE-NEOX-DEBUG] pos=%d, head=%d, i=%d, freqIndex=%d, cos=%.6f, sin=%.6f%n",
                            position, head, i, freqIndex, cos, sin);
                    }

                    // NEOX-style RoPE: rotate (x_i, x_{i+d/2}) pairs
                    int idx1 = headOffset + i;
                    int idx2 = headOffset + i + headDim / 2;

                    // Apply RoPE to query
                    float q0 = query[idx1];
                    float q1 = query[idx2];
                    query[idx1] = q0 * cos - q1 * sin;
                    query[idx2] = q0 * sin + q1 * cos;

                    // Apply RoPE to key
                    float k0 = key[idx1];
                    float k1 = key[idx2];
                    key[idx1] = k0 * cos - k1 * sin;
                    key[idx2] = k0 * sin + k1 * cos;

                    // Debug NEOX RoPE application for first few values
                    if (position <= 1 && head == 0 && i <= 2) {
                        System.err.printf("[ROPE-NEOX-APPLY] pos=%d, head=%d, i=%d: Q[%d,%.6f→%.6f] Q[%d,%.6f→%.6f]%n",
                            position, head, i, idx1, q0, query[idx1], idx2, q1, query[idx2]);
                        System.err.printf("[ROPE-NEOX-APPLY] pos=%d, head=%d, i=%d: K[%d,%.6f→%.6f] K[%d,%.6f→%.6f]%n",
                            position, head, i, idx1, k0, key[idx1], idx2, k1, key[idx2]);
                    }
                }
            }
        }
